import React, { useState, useEffect } from 'react';
import { MeetingService } from '../services/MeetingService';
import { AssembleiaData } from '../models/Meeting';

interface MeetingListProps {
  condominiumId: string;
  userId: string;
  userRole?: 'admin' | 'resident';
}

const MeetingList: React.FC<MeetingListProps> = ({ condominiumId, userId, userRole = 'resident' }) => {
  const [assembleias, setAssembleias] = useState<AssembleiaData[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editingAssembleia, setEditingAssembleia] = useState<AssembleiaData | null>(null);
  const [selectedStatus, setSelectedStatus] = useState('all');
  const [selectedType, setSelectedType] = useState('all');
  
  const meetingService = new MeetingService();

  useEffect(() => {
    loadAssembleias();
  }, [condominiumId, selectedStatus, selectedType]);

  const loadAssembleias = async () => {
    try {
      let filters: any = {};
      
      if (selectedStatus !== 'all') {
        filters.status = selectedStatus;
      }
      
      if (selectedType !== 'all') {
        filters.tipo = selectedType;
      }
      
      const assembleiasData = await meetingService.getAllAssembleias(condominiumId, filters);
      setAssembleias(assembleiasData);
    } catch (error) {
      console.error('Erro ao carregar assembleias:', error);
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir esta assembleia?')) {
      try {
        await meetingService.deleteAssembleia(id);
        loadAssembleias();
      } catch (error) {
        console.error('Erro ao excluir assembleia:', error);
      }
    }
  };

  const handleChangeStatus = async (id: string, status: string) => {
    try {
      await meetingService.changeAssembleiaStatus(id, status);
      loadAssembleias();
    } catch (error) {
      console.error('Erro ao alterar status da assembleia:', error);
    }
  };

  const formatDate = (date: Date): string => {
    return new Date(date).toLocaleString('pt-BR');
  };

  const getStatusLabel = (status: string): string => {
    switch(status) {
      case 'convocada': return 'Convocada';
      case 'em_andamento': return 'Em Andamento';
      case 'concluida': return 'Concluída';
      case 'cancelada': return 'Cancelada';
      default: return status;
    }
  };

  const getStatusColor = (status: string): string => {
    switch(status) {
      case 'convocada': return 'bg-blue-100 text-blue-800';
      case 'em_andamento': return 'bg-yellow-100 text-yellow-800';
      case 'concluida': return 'bg-green-100 text-green-800';
      case 'cancelada': return 'bg-red-100 text-red-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };

  const getTypeLabel = (type: string): string => {
    switch(type) {
      case 'ordinaria': return 'Ordinária';
      case 'extraordinaria': return 'Extraordinária';
      default: return type;
    }
  };

  if (loading) {
    return <div className="text-center py-4">Carregando assembleias...</div>;
  }

  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 space-y-4 md:space-y-0">
        <h1 className="text-2xl font-bold text-gray-800">Assembleias</h1>
        {userRole === 'admin' && (
          <button 
            onClick={() => {
              setEditingAssembleia(null);
              setShowForm(true);
            }}
            className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
          >
            Nova Assembleia
          </button>
        )}
      </div>

      <div className="mb-6 flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
        <div className="w-full md:w-1/2">
          <select
            value={selectedStatus}
            onChange={(e) => setSelectedStatus(e.target.value)}
            className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          >
            <option value="all">Todos os status</option>
            <option value="convocada">Convocada</option>
            <option value="em_andamento">Em Andamento</option>
            <option value="concluida">Concluída</option>
            <option value="cancelada">Cancelada</option>
          </select>
        </div>
        <div className="w-full md:w-1/2">
          <select
            value={selectedType}
            onChange={(e) => setSelectedType(e.target.value)}
            className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          >
            <option value="all">Todos os tipos</option>
            <option value="ordinaria">Ordinária</option>
            <option value="extraordinaria">Extraordinária</option>
          </select>
        </div>
      </div>

      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="min-w-full leading-normal">
          <thead>
            <tr className="bg-gray-100">
              <th className="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                Título
              </th>
              <th className="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                Tipo
              </th>
              <th className="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                Data
              </th>
              <th className="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                Local
              </th>
              <th className="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                Status
              </th>
              <th className="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                Ações
              </th>
            </tr>
          </thead>
          <tbody>
            {assembleias.length > 0 ? (
              assembleias.map((assembleia) => (
                <tr key={assembleia.id} className="border-b border-gray-200">
                  <td className="px-5 py-5 text-sm">
                    <div className="font-medium text-gray-900">{assembleia.titulo}</div>
                    <div className="text-gray-600 text-xs mt-1">{assembleia.descricao.substring(0, 50)}{assembleia.descricao.length > 50 ? '...' : ''}</div>
                  </td>
                  <td className="px-5 py-5 text-sm">
                    <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-purple-100 text-purple-800">
                      {getTypeLabel(assembleia.tipo)}
                    </span>
                  </td>
                  <td className="px-5 py-5 text-sm">
                    {formatDate(assembleia.dataAssembleia)}
                  </td>
                  <td className="px-5 py-5 text-sm">
                    {assembleia.local}
                  </td>
                  <td className="px-5 py-5 text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(assembleia.status)}`}>
                      {getStatusLabel(assembleia.status)}
                    </span>
                  </td>
                  <td className="px-5 py-5 text-sm">
                    <div className="flex space-x-2">
                      <button
                        onClick={() => {
                          setEditingAssembleia(assembleia);
                          setShowForm(true);
                        }}
                        className="text-blue-600 hover:text-blue-900"
                        title="Editar"
                      >
                        Editar
                      </button>
                      {userRole === 'admin' && (
                        <>
                          <select
                            value={assembleia.status}
                            onChange={(e) => handleChangeStatus(assembleia.id!, e.target.value)}
                            className="text-xs border rounded px-1 py-0.5"
                          >
                            <option value="convocada">Convocada</option>
                            <option value="em_andamento">Em Andamento</option>
                            <option value="concluida">Concluída</option>
                            <option value="cancelada">Cancelada</option>
                          </select>
                          <button
                            onClick={() => handleDelete(assembleia.id!)}
                            className="text-red-600 hover:text-red-900"
                            title="Excluir"
                          >
                            Excluir
                          </button>
                        </>
                      )}
                    </div>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan={6} className="px-5 py-5 text-sm text-center text-gray-500">
                  Nenhuma assembleia encontrada.
                </td>
              </tr>
            )}
          </tbody>
        </table>
      </div>

      {/* Modal para formulário */}
      {showForm && userRole === 'admin' && (
        <MeetingForm
          condominiumId={condominiumId}
          assembleia={editingAssembleia}
          onClose={() => {
            setShowForm(false);
            setEditingAssembleia(null);
          }}
          onSave={() => {
            loadAssembleias();
            setShowForm(false);
            setEditingAssembleia(null);
          }}
        />
      )}
    </div>
  );
};

export default MeetingList;